	...
	CLAY(CLAY_ID("ScreenTTSAsr"),
		...
		if (ctx->settings.is_module_tts) {
			...
			CLAY(CLAY_ID("TTSSectionHeader"),
				...
				draw_text(tr()->screen_tts_asr_tts_header, theme()->onSurface);
			}
			...
			CLAY(CLAY_ID("TTSInputSlot"),
				...
				auto tts_input_result = mobile_text_input(
					  ...
					  tr()->screen_tts_asr_tts_input_placeholder);
			}
		...
		if (ctx->settings.is_module_asr) {
			...
			CLAY(CLAY_ID("ASRSectionHeader"),
			     {.layout = {.sizing = {CLAY_SIZING_GROW(0),
			                            CLAY_SIZING_FIXED(section_height)}}}) {
				draw_text(tr()->screen_tts_asr_asr_header, theme()->onSurface);
			}

			bool is_asr_initialized =
				  ctx->audio_asr_tts_status.is_asr_initialized;
			// = UIAudioContext::TRUE ==
			...
			if (is_asr_initialized) {
				...
				CLAY(CLAY_ID("ASRSectionButtonSlot"),
					...
					if (buffer_size_bytes > 0) {
						auto push_text_to_tts_field_btn = mobile_icon_button(
							  ctx, CLAY_ID("PushTextToTTSField"),
							  Icons::CHEVRON_UP);
