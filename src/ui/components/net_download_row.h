...
inline void track_download(AppContext *ctx, Size req_pool_index,
                           StrView title) {
	...
	ctx->downloads.push(ctx->arena,
	                    {
							  .title = title,
							  .status = NetDownload::TRACKING,
							  .tracking_req_pool_index = req_pool_index,
							  .tracking_req_id = slot.req.request_id,
						});
}

inline bool net_download_row(AppContext *ctx, NetDownload &dl,
                             bool is_cancelable = false) {
	bool is_active = (dl.tracking_req_pool_index >= 0 &&
	                  dl.status == NetDownload::TRACKING);
	...
	if (is_active) {
		...
		case NetRequest::STATUS_FINISHED:
			...
			dl.status = NetDownload::FINISHED_OK;
			dl.tracking_req_pool_index = -1;
			is_active = false;
			break;
		case NetRequest::STATUS_CANCELLED:
			dl.status = NetDownload::FINISHED_CANCELLED;
			dl.tracking_req_pool_index = -1;
			is_active = false;
			break;
		case NetRequest::STATUS_ERROR:
			dl.error = req.error;
			dl.status = NetDownload::FINISHED_ERROR;
			dl.tracking_req_pool_index = -1;
			is_active = false;
			break;
	...
	CLAY(CLAY_IDI("DownloadRow", dl.tracking_req_id),
		...
		CLAY(CLAY_IDI("TextColumn", dl.tracking_req_id),
			...
			// }
			bool has_percentage =
				  dl.status == NetDownload::TRACKING && percentage >= 0;
		...
		if (is_cancelable) {
			...
			if (!is_active) {
				...
				style.text = dl.status == NetDownload::FINISHED_OK
				                   ? theme()->onRightContainer
				             : dl.status == NetDownload::FINISHED_ERROR
				                   ? theme()->onWrongContainer
				                   : theme()->primary;

				icon = dl.status == NetDownload::FINISHED_OK
				             ? Icons::CIRCLE_CHECK
				             : Icons::ROTATE_LEFT;
			}
