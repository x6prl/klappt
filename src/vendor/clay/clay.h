...
// Sets the DPI / display scale used for scroll gesture thresholds. Values <= 0 fall back to 1.0.
CLAY_DLL_EXPORT void Clay_SetDpiScale(float dpiScale);
...
Clay_Context* Clay_Initialize(Clay_Arena arena, Clay_Dimensions layoutDimensions, Clay_ErrorHandler errorHandler) {
    ...
    *context = CLAY__INIT(Clay_Context) {
        ...
        .dpiScale = oldContext && oldContext->dpiScale > 0 ? oldContext->dpiScale : 1.0f,
        .internalArena = arena,
    };
...
CLAY_WASM_EXPORT("Clay_SetDpiScale")
void Clay_SetDpiScale(float dpiScale) {
    Clay_Context* context = Clay_GetCurrentContext();
    if (!context) {
        return;
    }
    context->dpiScale = dpiScale > 0 ? dpiScale : 1.0f;
}
...
void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta, float deltaTime) {
    Clay_Context* context = Clay_GetCurrentContext();
    float dpiScale = context->dpiScale > 0 ? context->dpiScale : 1.0f;
    float dragThreshold = 10.0f * dpiScale;
    float momentumStopThreshold = 1.0f * dpiScale;
    ...
    for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
        ...
        if (!isPointerActive && scrollData->pointerScrollActive) {
            float momentumTime = scrollData->momentumTime;
            if (momentumTime < 0.016f) {
                momentumTime = 0.016f;
            }
            float xDiff = scrollData->scrollPosition.x - scrollData->scrollOrigin.x;
            if (xDiff < -dragThreshold || xDiff > dragThreshold) {
                scrollData->scrollMomentum.x = (scrollData->scrollPosition.x - scrollData->scrollOrigin.x) / (momentumTime * 30);
            }
            float yDiff = scrollData->scrollPosition.y - scrollData->scrollOrigin.y;
            if (yDiff < -dragThreshold || yDiff > dragThreshold) {
                scrollData->scrollMomentum.y = (scrollData->scrollPosition.y - scrollData->scrollOrigin.y) / (momentumTime * 30);
            }
        ...
        scrollData->scrollMomentum.x *= 0.88f;
        bool scrollOccurred = scrollDelta.x != 0 || scrollDelta.y != 0;
        if ((scrollData->scrollMomentum.x > -momentumStopThreshold && scrollData->scrollMomentum.x < momentumStopThreshold) || scrollOccurred) {
            scrollData->scrollMomentum.x = 0;
        }
        float minScrollX = -(CLAY__MAX(scrollData->contentSize.width - scrollData->layoutElement->dimensions.width, 0));
        scrollData->scrollPosition.x = CLAY__MIN(CLAY__MAX(scrollData->scrollPosition.x, minScrollX), 0);
        if (scrollData->scrollPosition.x == 0 || scrollData->scrollPosition.x == minScrollX) {
            scrollData->scrollMomentum.x = 0;
        }

        scrollData->scrollPosition.y += scrollData->scrollMomentum.y;
        scrollData->scrollMomentum.y *= 0.88f;
        if ((scrollData->scrollMomentum.y > -momentumStopThreshold && scrollData->scrollMomentum.y < momentumStopThreshold) || scrollOccurred) {
            scrollData->scrollMomentum.y = 0;
        }
        float minScrollY = -(CLAY__MAX(scrollData->contentSize.height - scrollData->layoutElement->dimensions.height, 0));
        scrollData->scrollPosition.y = CLAY__MIN(CLAY__MAX(scrollData->scrollPosition.y, minScrollY), 0);
        if (scrollData->scrollPosition.y == 0 || scrollData->scrollPosition.y == minScrollY) {
            scrollData->scrollMomentum.y = 0;
        }
